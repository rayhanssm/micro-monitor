  ...
  return (
    <nav className="px-4 lg:px-[116px] py-4 w-full bg-white fixed backdrop-blur-2xl flex items-center justify-between z-10">
      ...
      <div className={`block lg:hidden`}>
        <button onClick={() => setIsOpenNavMobile(true)}>
          <Menu color="#0F766E" />
        </button>
        <div
          ref={navMobileRef}
          className={`
            ${
              isOpenNavMobile
                ? `scale-100 bg-white lg:invisible`
                : `scale-90 invisible`
            } absolute border-2 mt-2 right-4 p-4 w-[212px] gap-8 flex flex-col rounded-lg shadow-md transition-all z-50
          `}
        >
            ...
            <button
              className={
                currPath === paths.settings
                  ...
                  : "border rounded-md px-4 py-2 transition-all"
              }
              onClick={() => {
                push(paths.settings);
                setIsOpenNavMobile(false);
